...
set_numbers[0] = 30;//FoodStorage COutpost NW
set_numbers[1] = 27;//FoodStorage
set_numbers[2] = 26;//FoodStorage IOutpost SE
set_numbers[3] = 33;//FoodStorage GOutpost SW
set_numbers[4] = 28;//FoodStorage GOutpost NE
set_numbers[5] = 25;//WatchTower
set_numbers[6] = 36;//GuardTower
set_numbers[7] = 39;//Numantia IOutpost N
set_numbers[8] = 38;//Numantia IOutpost S
set_numbers[9] = 37;//Numantia village
set_numbers[10] = 23;//WatchTower GOutpost E
set_numbers[11] = 24;//WatchTower IOutpost SE
...
while (NO_40.obj.player == 4) {
	...
	if (timer <= 0) {
		timer = attack_interval;
		SpawnGroup("I_OuterWave1");
	}
	all_to_iberia = true;
	for (i = 0; i < 34; i += 1) {
		n = set_numbers[i];
		siege_group_name = "I_OuterWave1_S_" + n;
		if (GetNamedObj("NO_" + n).obj.player == 1) {
			all_to_iberia = false;
			if (!I_OuterWave1.IsEmpty()) {
				I_OuterWave1.AddToGroup(siege_group_name);
				I_OuterWave1.RemoveFromGroup("I_OuterWave1");
			...
			if (!Group(siege_group_name).IsEmpty()) {
				Group(siege_group_name).AddToGroup("I_OuterWave1");
				Group(siege_group_name).RemoveFromGroup(siege_group_name);
			}
		}
	}
	if (all_to_iberia)
		I_OuterWave1.SetCommand("advance", R_Scipio.obj.pos);
}